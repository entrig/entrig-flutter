...
- Added null safety to `NotificationEvent.fromMap` to prevent crashes on missing
  title/body fields

## 0.0.8-dev

- Fixed iOS foreground notification display not respecting
  `showForegroundNotification` setting

## 0.0.7-dev

- Added `showForegroundNotification` option to control whether notifications are
  displayed when app is in foreground

## 0.0.6-dev

- **BREAKING**: Removed `AutoRegisterWithSupabaseAuth` to eliminate Supabase
  dependency
...
- Updated to identify debug device tokens when register for Testing from
  dashboard

## 0.0.2-beta

- Enhanced iOS setup command to automatically update Info.plist and
  Runner.entitlements
